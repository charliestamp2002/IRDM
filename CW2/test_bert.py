...
class PairwiseBERTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        passage = row['passage']
        label = row['relevancy']
        inputs = tokenizer(query, passage, padding='max_length', truncation=True, max_length=128, return_tensors='pt')
        return {
...
class SingleQueryBERTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pid = str(self.passages.iloc[idx]["pid"])
        inputs = tokenizer(self.query, passage, padding='max_length', truncation=True, max_length=128, return_tensors='pt')
        return {
            'input_ids': inputs['input_ids'].squeeze(0),
            'attention_mask': inputs['attention_mask'].squeeze(0),
            'pid': pid
...
def compute_bert_scores(model, queries_df, candidates_df, include_relevancy=False, batch_size=32):
    ...
    for _, query_row in tqdm(queries_df.iterrows(), total=len(queries_df), desc="Scoring Queries"):
        qid = query_row["qid"]
...
if __name__ == "__main__":
    print("Running Pairwise BERT Debug Test...")

    dtype_mapping = {"qid": str, "pid": str, "query": str, "passage": str, "relevancy": int}

    # Load small samples of training/validation data
    train_df = pd.read_csv("train_data.tsv", sep="\t", names=["qid", "pid", "query", "passage", "relevancy"], header=0, dtype=dtype_mapping)
    positive_samples = train_df[train_df['relevancy'] == 1].sample(n=3, random_state=42)
    negative_samples = train_df[train_df['relevancy'] == 0].sample(n=9, random_state=42)
    train_df = pd.concat([positive_samples, negative_samples]).reset_index(drop=True)

    val_df = pd.read_csv("validation_data.tsv", sep="\t", names=["qid", "pid", "query", "passage", "relevancy"], header=0, dtype=dtype_mapping)
    positive_val_samples = val_df[val_df['relevancy'] == 1].sample(n=5, random_state=42)
    negative_val_samples = val_df[val_df['relevancy'] == 0].sample(n=10, random_state=42)
    val_df = pd.concat([positive_val_samples, negative_val_samples]).reset_index(drop=True)

    train_loader = DataLoader(PairwiseBERTDataset(train_df), batch_size=8, shuffle=True)
    val_loader = DataLoader(PairwiseBERTDataset(val_df), batch_size=8)
    ...
    for epoch in range(1):  # Only 1 epoch for debug
        print(f"Epoch {epoch+1}")
        train(model, train_loader, optimizer, criterion, device)

    # Load and subset test queries for quick ranking
    test_queries = pd.read_csv("test-queries.tsv", sep="\t", names=["qid", "query"])
    candidates = pd.read_csv("candidate_passages_top1000.tsv", sep="\t", names=["qid", "pid", "query", "passage"])

    test_queries = test_queries.head(2)  # Only test on first 2 queries
    candidates = candidates[candidates["qid"].isin(test_queries["qid"])]

    test_bert_rankings = compute_bert_scores(model, test_queries, candidates)
    save_rankings(test_bert_rankings, "test_bert.txt")
    print(test_bert_rankings.head(10))

    # Sample a small validation set for fast testing
    val_queries_df = val_df[["qid", "query"]].drop_duplicates().head(5)
    val_passages_df = val_df[["qid", "pid", "passage", "relevancy"]]
    val_passages_df = val_passages_df[val_passages_df["qid"].isin(val_queries_df["qid"])]

    print("\nRecomputing rankings on small validation set for mAP/NDCG testing...")
    metrics = compute_mAP_and_mNDCG(
        model=model,
        queries_df=val_queries_df,
        passages_df=val_passages_df,
        include_relevancy=True
    )

    print(f"✅ Test Metrics - mAP: {metrics['mAP']:.4f}, mNDCG: {metrics['mNDCG']:.4f}")