...
class TwoTowerNet(nn.Module):
    def __init__(self, embedding_dim, use_cosine = False):
        ...
        # Final scoring layer using similarity features + abs diff
        final_input_dim = 3 + 256  # 3 similarity features + abs_diff
    ...
    def forward(self, x):
        ...
        dot_prod = torch.sum(q_out * p_out, dim=1, keepdim=True)  # (batch_size, 1)
        q_norm = torch.norm(q_out, dim=1, keepdim=True)
        p_norm = torch.norm(p_out, dim=1, keepdim=True)
        euclidean_dist = torch.norm(q_out - p_out, dim=1, keepdim=True)
        cosine_sim = (dot_prod / (q_norm * p_norm + 1e-8)).clamp(-1, 1)

        # Absolute difference vector
        abs_diff = torch.abs(q_out - p_out)  # (batch_size, 256)

        # Concatenate all features
        features = torch.cat([dot_prod, euclidean_dist, cosine_sim, abs_diff], dim=1)  # (batch_size, 259)
        score = self.final_layers(features)  # (batch_size, 1)
...
if __name__ == "__main__": 
    ...
    rankings_df = compute_tt_scores(model, test_queries_df, candidates_df, neural_network_type="NN")

    # Save rankings
    save_rankings(rankings_df, "NN.txt")
    # output_file = "NN.txt"
